...
pub trait Arena {
    ...
    fn create_game(&self) {
        ...
        require!(payments.len() == 2, "Expected soldier NFT and entrance fee");

        let soldier = &payments.get(0);
        let entrance_fee = &payments.get(1);
        ...
        self.games(&game_id).set(&ArenaGame {
            ...
            entrance_fee: entrance_fee.amount.clone(),
            competitor: None,
...
fn join_game(&self, game_id: u64) {
    let payments = self.call_value().all_esdt_transfers();
    let soldier = &payments.get(0);
    let entrance_fee = self.call_value().egld_value().into_big_int();
    
    self.validate_soldier(soldier);

    let mut game = self.games(&game_id).get();
    require!(game.competitor.is_none(), "Game already has a competitor");
    require!(entrance_fee == game.entrance_fee, "Invalid entrance fee");
