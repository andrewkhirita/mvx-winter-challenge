...
pub trait Stone {
    #[init]
    fn init(&self) {
        self.token_id().set(&TokenIdentifier::from("STONE"));
    }
    ...
    fn issue_token(&self, name: ManagedBuffer, ticker: ManagedBuffer) {
        ...
        self.token_manager().issue_and_set_all_roles(amount.clone_value(), name, ticker, 0, Some(self.callbacks().stone_token_callback()));
    }
